...
\includeplot{berry_diff}{The function $f(x) = \P{\BBs \le x} - \fPhi{x}$ for $n=1000$ and $p=\frac = 14$. You see that the maximum error occurs for $x\approx 0$. The function shape reminds the shape of $g(x)=\br{x^2-1} e^{-x^2}$ which leads to the first term of the Edgeworth expansion as we will see in the next chapter.}
...
Today is the theorem of de Moivre and Laplace taught in many courses and occurs in many textbooks (such as ...). It serves as an example of the central limit theorem and it is used in many exercises for students. Computerprogramme
