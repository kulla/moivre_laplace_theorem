...
\begin{proof}
  ...
  Because $\tilde s$ is in the interval bounded by $p$ and $s=\frac kn = p + \littleo{1}$ we deduce $\tilde s \in p + \littleo{1}$, too. Similarly we get $1-\tilde s\in q+\littleo{1}$ because $1-s = \frac{n-k}{n} \in q+\littleo{1}$. From this follows:
...
\begin{proof}
  ...
  \end{align}
  The first sum can be estimated with the Chebyshev's inequality \cite{wiki:chebyshev}:
  \begin{align}
...
\includeplot{edgeworth_diff_cdf}{This plot shows some values of $f(z) = \P{\BBs \le z} -\fPhi{z}$ and $g(z)=\frac{q-p}6 \br{1-z^2}\fphi{z} h + \Ro{np+z\sqrt{npq}} \fphi{z} h $ for $n=1000$ and $p=\frac 14$. The fact that the values of both functions coincide demonstrates that we have found a good approximation.}

These extensions of the normal approximation are precisely the first two terms of the Edgeworth Expansion  which is a series to approximates the cumulative distribution function of a random variable \cite[pp. 39 ff.]{hall}\cite{wiki:edgeworth}. In the case of the binomial distribution the Edgeworth Expansion is \cite[p. 46]{hall}
\begin{align}
