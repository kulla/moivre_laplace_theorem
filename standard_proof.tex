...
In this section we will show that the probability mass function of the standardized binomial distribution can be approximated by the density function of the normal distribution. This will be the basis for the global version of de Moivre-Laplace theorem. \todo{cite Iosif}
...
\begin{proof}
  ...
  Because \todo{$1+a_n \in \fexp{a_n + \bigo{a_n^2}}$} we have
...
\todo{Chernoff bound without proof}
...
\begin{proof}
  Let $\seq{c_n}$ be \todo{Was?}. We split the sum $\sum_{a \le \x \le b} \bb{\x}$ into two sums:
  ...
  \todo{Kontrollieren: Wirklich richtige Formel?!} \todo{$\bigabs{\cdot}$ definieren} \todo{$\bigabs{\cdot}$ überall einbauen}
  ...
  Now we can evaluate the Riemann sum $\sum_{a \le \x \le b} \f{\x} h$. We define $\tilde a = \inf \{ \x : \x \ge a \}$ and $\tilde b = \inf \{ \x : \x \le b \}$. Note that the difference of $a$ and $\tilde a$ is less than or equal $h$ and the distance of $a$ and $\tilde a - \frac h2$ can be estimated by $\frac h2$. Similarly we have $\abs{\tilde b-b} \le h$ and $\abs{\br{\tilde b+\frac h2}-b} \le \frac h2$. \todo{better proof of estimates} We have \todo{$\x[0]$ must be smaller $a$ and $\x[n]$ must be greater $b$}
  ...
  \todo{Rules: $\bigabs{\bigabs{\epsilon}} = \bigabs{\epsilon}$, $c\cdot \bigabs{\epsilon} = \bigabs{|c|\epsilon}$, $\int_a^{b+\bigabs{\epsilon}} f(t) dt = \int_a^b f(t) dt + \bigabs{\snorm{f} \epsilon}$}
  ...
  \begin{align}
    ...
    \begin{comment}
      \begin{aligned}
        ...
        \sum_{a \le \x \le b} \frac{q-p}6 \br{\x^3-\x} \fphi{\x} h^2 & \in \frac{q-p}6 \int_a^b \br{t^3-t} \fphi{t} \d{t} h + \bigo{h^2} \bnl
