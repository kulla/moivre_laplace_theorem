...
\begin{proof}
  ...
  \begin{align}
    ...
    &= -\x[k]\bb{\x}h + \bb{\x[k]} \cdot \bigo{\pol{\x}h^2}
  \end{align}
  So we can conclude
  \begin{align}
    \bb{\x[k+1]} &= \bb{\x[k]} -\x[k]\bb{\x[k]} h + \bb{\x[k]} \cdot \bigo{\pol{\x}h^2} \bnl
    &
    \begin{comment}
      \text{Set } f(x,y) = -xy
    \end{comment} \bnl
    &= \bb{\x[k]} - \br{\f{\x[k], \bb{\x[k]}} + \bb{\x[k]} \cdot \bigo{\pol{\x}h}}h \bnl
    &
    \begin{comment}
      \text{Set } M(x,y,h,f) = f(x,y) + \bb{\x[k]} \cdot \bigo{\pol{\x}h}
    \end{comment} \bnl
    &= \bb{\x[k]} + M(\x[k], \bb{\x[k]}, f, h) h
  \end{align}

  Therefore we can think about $\bb{\x}$ as the results of the one-step method $M(x,y,f,h)=f(x,y)+\bb{\x[k]} \cdot \bigo{\pol{\x}h}$ whereby we can choose any $\bb{\x}$ as the initial value. We take $\bb{\xnp}$ for $\knp=\rnd{np}$ so that $\xnp$ is the $\x$ which is closest to $0$. The one-step method now corresponds to the ODE with initial condition
  \begin{align}
  ...
  In order to estimate the error $\abs{\bb{\x}-\phin{\x}}$ with theorem \ref{thm:ode_global_error} we need restrict $f$ to the domain $I\times \R$ with $I=\interval{-\abs{\x}}{\abs{\x}}$ so that each polynomial on $I$ is bounded and $\x$ as well as $\xnp$ are elements of $I$. We find by using the Taylor series
  \begin{align}
