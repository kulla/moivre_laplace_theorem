...
The binomial distribution occurs often in statistics and thus in all fields which use statistical tools or probability theory like biology, psychology, economics, physics etc. \todo{cite}. An application in gambling we have already met in the beginning section of this chapter.

The binomial distribution occurs often in counting processes to calculate the odds of having $k$ times a certain property among $n$ objects / occasions. The binomial distribution is also used as a simplified model when the probability of having this certain properties varies a little bit or is not independent between all objects / occasions. Take the following example:
...
Firstly it is likely that the probability of having a faulty product varies over time. For example it may depend on the temperature of the machines which may fluctuate. Secondly the occurrence of a badly made product may increase the likelihood for flawed products for the next produced merchandises. There may be a failure in one of the machines which also effects the next products until the failure is corrected. 

Although the premises for the binomial distribution is not fulfilled in this example, this distribution may be used by the company to calculate with. This shows how important the binomial distribution for contemporary statistical analysis is because it may be used whenever one investigate a counting process.
