...
Because we will use notations such as $\bigo{\cdot}$ or $\bigabs{\cdot}$, we will have to deal with expressions containing sets of sequences instead of members of a concrete sequence. Thus we have to define rules what how to operate with sets. Here we need to find a definition so that the result is the best approximation for the result of the considered number\footnote{cf. \cite[pp. 17-18]{kulla}}.
...
I will use variables like $h=\frac{1}{\sqrt{npq}}$ where I drop the dependence on $n$ in the name of the variable. When I state something like $\littleo{h}$ I will always mean $\littleo{h_n}$, e.g.
\begin{align}
